...
const UserQuiz = ({ quiz, ...props }) => {
    ...
    server
      ...
      .then(response => {
        server.get('/quizzes').then(({ data }) => {
          setUserQuizzes(
            data.filter(quiz => quiz.author === user.username).sort((a, b) => b.id - a.id)
          )
  ...
  const footer = (
    <div>
      ...
      <Button label="No" icon="pi pi-times" onClick={() => setModalVisable(false)} secondary />
    </div>
  )
  ...
  return (
    <Wrapper>
      ...
      >
        Are you sure you'd like to delete your quiz {quiz.title}? This action cannot be undone.
      </Dialog>
      <InnerWrapper>
        ...
        <FooterWrapper>
          <div>
            <a href="#questions" style={{ cursor: 'default', fontWeight: 'bold' }}>
              {quiz.question_count} questions
            </a>
            <span style={{ padding: '0 4px' }}>&#8226;</span>
            <a href="#votes" style={{ cursor: 'default', fontWeight: 'bold' }}>

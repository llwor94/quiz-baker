...
const Quiz = ({ quiz, ...props }) => {
  ...
  return (
    <Wrapper>
      ...
      {user && user.username === quiz.author && props.history.location.pathname === '/quizzes' && (
      ...
      <div style={{ display: 'flex' }}>
        ...
        <InnerWrapper>
          <div>
            <Header>
              <div>
                <Title onClick={() => props.history.push(`quizzes/${quiz.id}`)}>{quiz.title}</Title>
            ...
            <UserNameWrapper className="icon-wrapper">
              ...
              {user ? (
                <FontAwesomeIcon
                  ...
                  onMouseEnter={e => animateIn(e, props.theme.aqua)}
                  onMouseLeave={e => animateOut(e, quiz.favorite ? '#875818' : '#b2b2b2')}
                />
              ) : (
                <div />
              )}
